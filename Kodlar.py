...
print("Regresyon modeli Y = {} + {}*x".format(lr.intercept_[0].round(2), lr.coef_[0][0].round(2)))
...
print("Regresyon modeli Y = {} + {}*x".format(b0, b1))
tahmin_leo_yasi=np.array([24 , 32 , 45 , 50 , 55])
tahmin_gf_yasi = lr.predict(tahmin_leo_yasi.reshape(-1,1))
plt.plot(random_x, b0 + b1*random_x, color='red', label='regresyon' )
plt.scatter(tahmin_leo_yasi,tahmin_gf_yasi.reshape(-1,1) )
plt.legend()
plt.xlabel('Leo Age')
plt.ylabel('GF Age')
